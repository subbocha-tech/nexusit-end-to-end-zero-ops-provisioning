...
export function HomePage() {
  ...
  const stats = [
    { 
      ...
      value: `$${licenses.reduce((sum, l) => sum + l.monthlyCost, 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 
      icon: CreditCard, 
      color: 'text-blue-600', 
      trend: '+18.2%' 
    },
    ...
    { 
      label: t('dashboard.stats.utilization'), 
      value: '94.8%', 
      icon: ShieldCheck, 
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12">
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat, i) => (
            <Card key={i} className="border-border/50 shadow-sm overflow-hidden group hover:border-primary/20 transition-colors">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className={cn(
                    ...
                    stat.trend.startsWith('+') ? "text-emerald-600 bg-emerald-50" : "text-orange-600 bg-orange-50"
                  )}>
                    {stat.trend}
                  </span>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {feedItems.slice(0, 8).map((item) => (
                  <div key={item.id} className="flex items-start justify-between group">
                    <div className="flex items-start gap-4">
                      <div className="mt-1 h-8 w-8 rounded-lg bg-accent/50 flex items-center justify-center shrink-0">
                    ...
                    <div className="text-right flex flex-col items-end gap-1.5 shrink-0">
                      ...
                      {item.rawStatus ? (
                         <Badge variant="outline" className={cn(
                          ...
                          item.rawStatus === 'rejected' && "bg-red-50 text-red-700 border-red-200"
                        )}>
                          {item.rawStatus}
                        </Badge>
                      ) : (
          ...
          <div className="space-y-6">
            <Card className="border-border/50 shadow-sm bg-blue-600 text-white border-none overflow-hidden relative group">
              ...
              <CardContent className="space-y-3">
                <Link to="/catalog">
                  <Button className="w-full bg-white text-blue-600 hover:bg-blue-50 border-none justify-start gap-2 h-11 transition-all">
                    <PlusCircle className="h-4 w-4" /> {t('dashboard.requestNewApp')}
                  </Button>
                </Link>
                <Button className="w-full bg-blue-500 text-white hover:bg-blue-400 border-none justify-start gap-2 h-11 transition-all">
